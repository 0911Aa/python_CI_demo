...
class ElementActions:
        ...
        with allure.step("检查：'{0}'".format(name)):
                try:
                    if element.startswith('//'):
                        return WebDriverWait(self.driver, 10).until(lambda driver: driver.find_element_by_xpath(element), '失败')
                    else:
                        return WebDriverWait(self.driver, 10).until(lambda driver: driver.find_element_by_id(element),'失败')
    ...
    #     allure.attach(pngbyte, locator.get('name'), allure.attach_type.PNG)
